...
def save(best_model, dataname):

    path = './saves/'

    os.makedirs(path, mode=0o777, exist_ok=True)

    torch.save(best_model, path+'model_{}.pth'.format(dataname))
...
def dataloader(datadir, dataname, seq_len, batch_size, num_workers, test=False):
    ...
    if dataname == 'atac':
        ...
        if test:
            ...
            test_data = pd.read_csv(
                datadir + "/ATAC_test_regions.bed.gz",
                sep='\t', 
                names=("chrom","start","end")
                )
            
            test_dataset = BedPeaksDatasetTest(test_data, genome, seq_len) 
        ...
        if test:
            ...
            test_dataset = BedPeaksDataset(test_data, genome, seq_len)
...
def run_one_epoch(train_flag, dataloader, model, optimizer, device="cuda"):
    ...
    for (x,y) in dataloader: # collection of tuples with iterator
        ...
        accuracy = pr_auc(output, y) #we measure pr_auc not accuracy
